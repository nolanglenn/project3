...
class Button extends React.Component {
    ...
    setCorrectStatus() {
        const userStatus = ['Take Job', 'Job Complete', 'Awaiting Payment', 'Payment Recieved']
        const ownerStatus = ['Awaiting Someone to Take Job', 'Awaiting Completion of Job', 'Payment Sent', 'Close Job']
        ...
        if(this.state.user === this.state.originUser) {
            switch(this.state.clickCount) {
                case 0: {
                    ...
                    this.setState({OwnerButtonClass: "waves-effect btn-small disabled"})
                    console.log(this.state)
                    break;
                }
                case 1: {
    ...
    render() {
        ...
        return (
            <div className='container'>
                {this.state.status === 'Job Complete' ? <button style={{margin: '2.5px'}} onClick={this.goBack} className="waves-effect btn-small red">Cancel Job</button> : null}
                <button style={{margin: '2.5px'}} onClick={this.statusChange} value={this.state.status} className={this.state.user === this.state.originUser ? this.state.OwnerButtonClass : this.state.UserButtonClass}>{this.state.status}</button>
            </div>
        )
    }
}
